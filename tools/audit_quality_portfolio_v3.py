...
LEGACY_PATHS = (
    ...
    ".github/workflows/refresh-generator.yml",
)
ALLOWED_EXTRA_TESTS = {
    "crap4rust": {
...
def check_project_metadata(repository: str, actual: Path) -> dict[str, object]:
    ...
    if project.get("license") != "MIT":
        raise AuditError(f"{repository}: SPDX license metadata is not MIT")
    if project.get("license-files") != ["LICENSE"]:
        raise AuditError(f"{repository}: license-files must contain only LICENSE")
    ...
    if any(value.startswith("License ::") for value in classifiers):
        raise AuditError(f"{repository}: deprecated license classifier remains")
    return project


def language_for(repository: str) -> str:
    for prefix in ("crap4", "mutate4", "dry4"):
        if repository.startswith(prefix):
            suffix = repository[len(prefix):]
            return LANGUAGE_BY_REPOSITORY_SUFFIX[suffix]
    raise AuditError(f"cannot determine target language for {repository}")


def check_target_fixture(repository: str, actual: Path, python: Path, workspace: Path) -> None:
    language = language_for(repository)
    fixture = actual / "tests" / "fixtures"
    if language == "typescript":
        command = [
            ...
            (
                ...
                "p=Path('tests/fixtures/sample.ts'); "
                "assert not get_parser('typescript').parse(p.read_bytes()).root_node.has_error"
            ),
        ]
    elif language == "python":
        command = [str(python), "-m", "py_compile", str(fixture / "sample.py")]
    elif language == "rust":
        command = ["rustc", "--crate-type", "lib", str(fixture / "sample.rs"), "-o", str(workspace / f"{repository}.rlib")]
    elif language == "swift":
        command = ["swiftc", "-parse", str(fixture / "sample.swift")]
    elif language == "objective-c":
        command = ["clang", "-x", "objective-c", "-fsyntax-only", str(fixture / "sample.m")]
    elif language == "bash":
        command = ["bash", "-n", str(fixture / "sample.sh")]
    elif language == "c":
        command = ["cc", "-fsyntax-only", str(fixture / "sample.c")]
    elif language == "cpp":
        command = ["c++", "-std=c++20", "-fsyntax-only", str(fixture / "sample.cpp")]
    else:
        raise AuditError(f"unsupported target language: {language}")
    run(command, cwd=actual, timeout=300)
...
def install_and_test(
    ...
    check_target_fixture(repository, actual, python, workspace)
...
def audit_repository(
    ...
    install_and_test(repository, actual, wheel_dir, python, scripts, workspace)
