...
"###,
        "",
        "manual feature injection",
    );

    let cargo_cfg = r###"fn parse_cfg_output(text: &str, include_tests: bool) -> CfgContext {
...
"###;
    let index = scope.find(test_anchor).expect("missing scope test anchor");
